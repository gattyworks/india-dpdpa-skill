...
How to install the `dpdp-india` skill, invoke it, and run it outside Claude Code. The skill audits an app, codebase, or data flow against India's **DPDP Act 2023** (Act 22 of 2023) and the **DPDP Rules 2025** (notified 13 Nov 2025, G.S.R. 846(E)). It is an engineering aid for spotting likely gaps - not legal advice.
...
This registers two slash commands (`/dpdp-audit`, `/dpdp-update-check`) and the `dpdp-india` skill, which also activates from natural language.

## Commands

| Command | Argument | Default | Does |
|---|---|---|---|
| `/dpdp-audit` | path or scope | current repo / working directory | Runs a full compliance audit and returns the report |
| `/dpdp-update-check` | none | - | Re-fetches the pinned legal sources and flags any that changed |

### `/dpdp-audit [path or scope]`

The argument is a path or a free-text scope. If empty, the skill audits the current repository.

```bash
/dpdp-audit .                       # audit the whole repo
/dpdp-audit ./api                   # audit a subtree
/dpdp-audit "the signup + analytics flow"   # scope by description
```
...
1. **Copy the folder.** Drop [`plugins/dpdp-india/skills/dpdp-india/`](../plugins/dpdp-india/skills/dpdp-india/) into any agent that reads `SKILL.md` plus markdown references - Cursor, Windsurf, the Claude Agent SDK, or your own RAG pipeline. The folder is self-contained: the Act and Rules text, penalty schedule, audit checklist, code-detection patterns, GDPR↔DPDP map, and 13 policy templates all travel with it.
2. **Point the tool at the file.** Tell the model: *"read [SKILL.md](../plugins/dpdp-india/skills/dpdp-india/SKILL.md) and audit this codebase against it."* Everything it needs to follow the four-pass method is in the file and the `references/` it links.

| Harness | How |
|---|---|
| Cursor / Windsurf | Add the `skills/dpdp-india/` folder to the workspace or rules context; reference `SKILL.md` in the prompt |
...
The `${CLAUDE_PLUGIN_ROOT}` paths in `/dpdp-update-check` are Claude Code conventions; in other harnesses, run the script in `scripts/` directly.
