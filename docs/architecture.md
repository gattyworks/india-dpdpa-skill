...
`dpdp-india` is two things in one tree: a **Claude Code plugin** (installable from a marketplace) and a **portable AI skill** (self-contained markdown that runs in any harness). This page maps the directory layout, the three nested layers that wire it together, how Claude Code discovers and loads it, the progressive-disclosure design that keeps it fast, and why the skill folder is portable on its own.
...
```
dpdp-india/                                  # repo root
├── .claude-plugin/
│   └── marketplace.json                     # marketplace manifest → lists the plugin, points at ./plugins/dpdp-india
├── plugins/
│   └── dpdp-india/                          # the plugin
│       ├── .claude-plugin/
│       │   └── plugin.json                  # plugin manifest (name, description, version, author)
│       ├── README.md                        # plugin-level install/use/layout
│       ├── commands/                         # slash commands (auto-discovered)
│       │   ├── dpdp-audit.md                # /dpdp-audit [path] - invokes the skill's 4-pass method
│       │   └── dpdp-update-check.md         # /dpdp-update-check - runs the source-drift checker
...
│       └── skills/
│           └── dpdp-india/                   # the portable skill (self-contained)
...
|-------|---------------|------|
| 1. Marketplace | [`.claude-plugin/marketplace.json`](../.claude-plugin/marketplace.json) | Declares the `gattyworks-compliance` marketplace and its `plugins` array. The `dpdp-india` entry sets `source: "./plugins/dpdp-india"`, plus `version`, `description`, and `keywords`. India (DPDP) is first; the marketplace is built to hold more jurisdictions. |
| 2. Plugin | [`plugins/dpdp-india/.claude-plugin/plugin.json`](../plugins/dpdp-india/.claude-plugin/plugin.json) | The plugin manifest: `name`, `description`, `version` (1.0.0), `author`, `homepage`, `repository`, `license` (MIT), `keywords`. Its sibling folders - `commands/`, `scripts/`, `skills/` - are conventionally located and auto-discovered. |
| 3. Skill + commands + scripts | `skills/dpdp-india/`, `commands/`, `scripts/` | The behavior. The **skill** ([`SKILL.md`](../plugins/dpdp-india/skills/dpdp-india/SKILL.md) + `references/`) carries the audit method and the law. **Commands** are thin entry points that invoke the skill. **Scripts** keep the pinned sources honest. |
...
- **Commands** - `dpdp-audit.md` tells the agent to run the skill's four-pass method against a target path; `dpdp-update-check.md` runs the drift checker. Both are markdown with YAML front matter (`description`, `argument-hint`); `/dpdp-audit` consumes `$ARGUMENTS` as the audit scope.
...
reads .claude-plugin/marketplace.json
        │  plugins[].source = "./plugins/dpdp-india"
        ▼
/plugin install dpdp-india@gattyworks-compliance
        │
        ▼
reads plugins/dpdp-india/.claude-plugin/plugin.json   ← declares the plugin
        │
        ├── commands/  → /dpdp-audit, /dpdp-update-check   (auto-discovered)
        ├── skills/dpdp-india/SKILL.md                     (auto-discovered; loaded on trigger)
        └── scripts/   → resolved at run time via ${CLAUDE_PLUGIN_ROOT}
```

1. **`/plugin marketplace add gattyworks/dpdp-india`** points Claude Code at the repo. It reads `.claude-plugin/marketplace.json`, which lists `dpdp-india` with `source: "./plugins/dpdp-india"`.
2. **`/plugin install dpdp-india@gattyworks-compliance`** installs that plugin. Claude Code reads `plugins/dpdp-india/.claude-plugin/plugin.json` to register it.
...
flowchart TD
    ...
    subgraph L2["Layer 2 - Plugin"]
        PJ["plugins/dpdp-india/.claude-plugin/plugin.json"]
    end
    subgraph L3["Layer 3 - Behavior"]
        CMD["commands/<br/>/dpdp-audit · /dpdp-update-check"]
        SK["skills/dpdp-india/SKILL.md<br/>(entry point, small)"]
    ...
    MP -->|"source: ./plugins/dpdp-india"| PJ
    User -->|"/plugin install dpdp-india@gattyworks-compliance"| PJ
    PJ -->|auto-discover| CMD
    PJ -->|auto-discover| SK
    PJ -->|"${CLAUDE_PLUGIN_ROOT}"| SCR

    CMD -->|"/dpdp-audit invokes"| SK
    CMD -->|"/dpdp-update-check runs"| SCR
...
The `skills/dpdp-india/` folder is self-contained. It is plain markdown - `SKILL.md` plus the `references/` tree - with **no dependency on Claude Code, the marketplace, or the plugin manifests**. Any harness that can read a `SKILL.md` and follow its relative links to markdown references can run the audit.
...
- [`SKILL.md`](../plugins/dpdp-india/skills/dpdp-india/SKILL.md) - the skill entry point and reference map.
